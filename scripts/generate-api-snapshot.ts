...
// These symbols were removed from Askr's canonical route contract. Keep the
// website from reintroducing ambient-route guidance while older published
// artifacts remain installable during the staged clean break.
...
const versionsPath = resolve(root, 'src/pages/docs/package-versions.ts');
...
const installedPackageNames = Object.keys(packageLock.packages)
  ...
  .sort();
const packageNames = installedPackageNames.filter(
...
const versionsSource = formatGeneratedTypeScript(
  versionsPath,
  unformattedVersionsSource
);
...
if (process.argv.includes('--check')) {
  if (
    ...
    existingVersions !== versionsSource
  ) {
  ...
  await import('node:fs/promises').then(({ writeFile }) =>
    Promise.all([
      ...
      writeFile(versionsPath, versionsSource),
    ])
  );
  console.log(
