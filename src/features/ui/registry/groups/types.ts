...
const componentDescriptions: Record<string, string> = {
  button: 'Use Button for actions that submit, save, open, or mutate local state.',
  toggle: 'Use Toggle for a single pressed/unpressed setting such as bold, mute, or preview.',
  checkbox: 'Use Checkbox for one boolean form value or one item inside a checklist.',
  'visually-hidden':
  ...
  input: 'Use Input for short single-line text values such as names, filters, and URLs.',
  textarea:
    'Use Textarea for longer free-form text where line breaks matter.',
  field:
    'Use Field to group a label, control, hint, and validation message.',
  'radio-group':
  ...
  menu:
    'Use Menu for command lists where arrow-key navigation and item activation matter.',
  'dropdown-menu':
  ...
  stack:
    'Use Stack to create vertical rhythm between related elements.',
  inline:
  ...
  center:
    'Use Center to place one item in the middle of an available region.',
  spacer:
...
const stateGuidance: Partial<Record<string, string>> = {
  ...
  checkbox: 'Store checked state for forms or let the primitive manage default state.',
  input: 'Store the text value when validation, filtering, or submission needs it.',
  textarea:
  ...
  select: 'Store the selected value and render the selected label in the trigger.',
  'toggle-group':
  ...
  spinner: 'No value is required; render it while another operation is pending.',
};
...
export function getComponentUsage(component: UiComponentMeta): UiComponentUsage {
